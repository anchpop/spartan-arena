...
namespace Andre2
{
    ...
    public class APAgentSmith2 : Agent
    {
        ...
        {
            base.Start();
            APAgentSmith2.MEM.observedPositions = new List<BotPosition>();
            if (APAgentSmith2.MEM.observedSpawnPoints == null)
            {
                APAgentSmith2.MEM.observedSpawnPoints = new List<SpawnPointInfo>();
                APAgentSmith2.MEM.alreadyRandomized = false;
            }
            if (APAgentSmith2.MEM.spawnPointsVisited == null)
            {
                APAgentSmith2.MEM.spawnPointsVisited = new List<SpawnPointVisited>();
                APAgentSmith2.MEM.alreadyRandomized = false;
            }
        ...
        void RandomizeParameters()
        {
            ...
            Stats = "";
            if (!APAgentSmith2.MEM.alreadyRandomized)
            {
                ...
                APAgentSmith2.MEM.bulletsToDoOneDamageStored = bulletsToDoOneDamage;
                APAgentSmith2.MEM.snagRadiusStored = snagRadius;
                APAgentSmith2.MEM.healthPickupUtilityDangerStored = healthPickupUtilityDanger;
                APAgentSmith2.MEM.healthPickupUtilitySaferStored = healthPickupUtilitySafer;
                APAgentSmith2.MEM.healthPickupUtilitySafeStored = healthPickupUtilitySafe;
                APAgentSmith2.MEM.aggressivenessStored = aggressiveness;
                APAgentSmith2.MEM.healthPackAttractionStored = healthPackAttraction;
                APAgentSmith2.MEM.ammoPackAttractionStored = ammoPackAttraction;

                APAgentSmith2.MEM.alreadyRandomized = true;
            }
            else
            {
                bulletsToDoOneDamage = APAgentSmith2.MEM.bulletsToDoOneDamageStored;
                snagRadius = APAgentSmith2.MEM.snagRadiusStored;
                healthPickupUtilityDanger = APAgentSmith2.MEM.healthPickupUtilityDangerStored;
                healthPickupUtilitySafer = APAgentSmith2.MEM.healthPickupUtilitySaferStored;
                healthPickupUtilitySafe = APAgentSmith2.MEM.healthPickupUtilitySafeStored;
                aggressiveness = APAgentSmith2.MEM.aggressivenessStored;
                healthPackAttraction = APAgentSmith2.MEM.healthPackAttractionStored;
                ammoPackAttraction = APAgentSmith2.MEM.ammoPackAttractionStored;
            }
            ...
            {
                {
                    ...
                    {
                        APAgentSmith2.MEM.spawnPointsVisited = (from point in APAgentSmith2.MEM.spawnPointsVisited
                                                               where point.pos != nearPoints[0].pos
                                                               where point.time > Time.time - 60
                                                               select point).ToList();
                        APAgentSmith2.MEM.spawnPointsVisited.Add(nearPoints[0]);
                    }
                }
                    ...
                    switch (si.sense)
                    {
                        case SensoryInput.eSense.vision:
                            if (si.type == eSensedObjectType.enemy)
                            {
                                ...
                                if (!APAgentSmith2.MEM.observedPositions.Contains(info))
                                    APAgentSmith2.MEM.observedPositions.Add(info);
                            }
                            else if (si.type == eSensedObjectType.item && si.obj is PickUp)
                            {
                                ...
                                if (!APAgentSmith2.MEM.observedSpawnPoints.Contains(info))
                                {
                                    APAgentSmith2.MEM.observedSpawnPoints.Add(info);
                                }
                            }
                            break;
                    }
        ...
        ClosestPosToExploreAndUtility? getExplorationUtility()
        {
            ...
            var pointsToExplore = (from point in iPoints
                                   where !APAgentSmith2.MEM.observedSpawnPoints.Any(info => info.pos == point.pos)
                                   where !APAgentSmith2.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
        ...
        Vector3? Lead(BotPosition bot)
        {
            if (bot.seenLastFrame)
            {
                ...
                var observedData = (from botPosition in APAgentSmith2.MEM.observedPositions
        ...
        PickupToSeekAndUtility? getSeekPickupUtility()
        {
            ...
            var healthPickups = (from point in APAgentSmith2.MEM.observedSpawnPoints
                                 where point.typ == PickUp.eType.health
                                 where !APAgentSmith2.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
                                 select point.pos).ToList();
            var ammoPickups = (from point in APAgentSmith2.MEM.observedSpawnPoints
        ...
        void Hunt(EnemyToHuntAndUtility target)
        {
            ...
            var snaggablesWithinRange = (from point in APAgentSmith2.MEM.observedSpawnPoints
                                         where !APAgentSmith2.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
