...
namespace Andre6
{
    ...
    public class APAgentSmith6 : Agent
    {
        ...
        float bulletsToDoOneDamage = 2.2f;
        float snagRadius = 5.2f;
        float healthPickupUtilityDanger = 14.9f;
        float healthPickupUtilitySafer = 6.9f;
        float healthPickupUtilitySafe = 1.7f;
        float aggressiveness = 1.3f;
        float healthPackAttraction = 2.2f;
        float ammoPackAttraction = 1;

        private void Start()
        {
            base.Start();
            APAgentSmith6.MEM.observedPositions = new List<BotPosition>();
            if (APAgentSmith6.MEM.observedSpawnPoints == null)
            {
                APAgentSmith6.MEM.observedSpawnPoints = new List<SpawnPointInfo>();
                APAgentSmith6.MEM.alreadyRandomized = false;
            }
            if (APAgentSmith6.MEM.spawnPointsVisited == null)
            {
                APAgentSmith6.MEM.spawnPointsVisited = new List<SpawnPointVisited>();
                APAgentSmith6.MEM.alreadyRandomized = false;
            }
        ...
        void RandomizeParameters()
        {
            ...
            Stats = "";
            if (!APAgentSmith6.MEM.alreadyRandomized)
            {
                ...
                APAgentSmith6.MEM.bulletsToDoOneDamageStored = bulletsToDoOneDamage;
                APAgentSmith6.MEM.snagRadiusStored = snagRadius;
                APAgentSmith6.MEM.healthPickupUtilityDangerStored = healthPickupUtilityDanger;
                APAgentSmith6.MEM.healthPickupUtilitySaferStored = healthPickupUtilitySafer;
                APAgentSmith6.MEM.healthPickupUtilitySafeStored = healthPickupUtilitySafe;
                APAgentSmith6.MEM.aggressivenessStored = aggressiveness;
                APAgentSmith6.MEM.healthPackAttractionStored = healthPackAttraction;
                APAgentSmith6.MEM.ammoPackAttractionStored = ammoPackAttraction;

                APAgentSmith6.MEM.alreadyRandomized = true;
            }
            else
            {
                bulletsToDoOneDamage = APAgentSmith6.MEM.bulletsToDoOneDamageStored;
                snagRadius = APAgentSmith6.MEM.snagRadiusStored;
                healthPickupUtilityDanger = APAgentSmith6.MEM.healthPickupUtilityDangerStored;
                healthPickupUtilitySafer = APAgentSmith6.MEM.healthPickupUtilitySaferStored;
                healthPickupUtilitySafe = APAgentSmith6.MEM.healthPickupUtilitySafeStored;
                aggressiveness = APAgentSmith6.MEM.aggressivenessStored;
                healthPackAttraction = APAgentSmith6.MEM.healthPackAttractionStored;
                ammoPackAttraction = APAgentSmith6.MEM.ammoPackAttractionStored;
            }
            ...
            {
                {
                    ...
                    {
                        APAgentSmith6.MEM.spawnPointsVisited = (from point in APAgentSmith6.MEM.spawnPointsVisited
                                                               where point.pos != nearPoints[0].pos
                                                               where point.time > Time.time - 60
                                                               select point).ToList();
                        APAgentSmith6.MEM.spawnPointsVisited.Add(nearPoints[0]);
                    }
                }
                    ...
                    switch (si.sense)
                    {
                        case SensoryInput.eSense.vision:
                            if (si.type == eSensedObjectType.enemy)
                            {
                                ...
                                if (!APAgentSmith6.MEM.observedPositions.Contains(info))
                                    APAgentSmith6.MEM.observedPositions.Add(info);
                            }
                            else if (si.type == eSensedObjectType.item && si.obj is PickUp)
                            {
                                ...
                                if (!APAgentSmith6.MEM.observedSpawnPoints.Contains(info))
                                {
                                    APAgentSmith6.MEM.observedSpawnPoints.Add(info);
                                }
                            }
                            break;
                    }
        ...
        ClosestPosToExploreAndUtility? getExplorationUtility()
        {
            ...
            var pointsToExplore = (from point in iPoints
                                   where !APAgentSmith6.MEM.observedSpawnPoints.Any(info => info.pos == point.pos)
                                   where !APAgentSmith6.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
        ...
        Vector3? Lead(BotPosition bot)
        {
            if (bot.seenLastFrame)
            {
                ...
                var observedData = (from botPosition in APAgentSmith6.MEM.observedPositions
        ...
        PickupToSeekAndUtility? getSeekPickupUtility()
        {
            ...
            var healthPickups = (from point in APAgentSmith6.MEM.observedSpawnPoints
                                 where point.typ == PickUp.eType.health
                                 where !APAgentSmith6.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
                                 select point.pos).ToList();
            var ammoPickups = (from point in APAgentSmith6.MEM.observedSpawnPoints
        ...
        void Hunt(EnemyToHuntAndUtility target)
        {
            ...
            var snaggablesWithinRange = (from point in APAgentSmith6.MEM.observedSpawnPoints
                                         where !APAgentSmith6.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
