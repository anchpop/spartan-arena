...
namespace Andre10
{
    ...
    public class APAgentSmith10 : Agent
    {
        ...
        float bulletsToDoOneDamage = 4.5f;
        float snagRadius = 5;
        float healthPickupUtilityDanger = 16;
        float healthPickupUtilitySafer = 7.2f;
        float healthPickupUtilitySafe = .87f;
        float aggressiveness = .06f;
        float healthPackAttraction = 4.5f;
        float ammoPackAttraction = 1.4f;

        private void Start()
        {
            base.Start();
            APAgentSmith10.MEM.observedPositions = new List<BotPosition>();
            if (APAgentSmith10.MEM.observedSpawnPoints == null)
            {
                APAgentSmith10.MEM.observedSpawnPoints = new List<SpawnPointInfo>();
                APAgentSmith10.MEM.alreadyRandomized = false;
            }
            if (APAgentSmith10.MEM.spawnPointsVisited == null)
            {
                APAgentSmith10.MEM.spawnPointsVisited = new List<SpawnPointVisited>();
                APAgentSmith10.MEM.alreadyRandomized = false;
            }
        ...
        void RandomizeParameters()
        {
            ...
            Stats = "";
            if (!APAgentSmith10.MEM.alreadyRandomized)
            {
                bulletsToDoOneDamage += 2 * (Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f));
                snagRadius += 2 * (Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f));
                healthPickupUtilityDanger += 2 * (Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f));
                healthPickupUtilitySafer += 2 * (Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f));
                healthPickupUtilitySafe += 2 * (Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f));
                aggressiveness += 2 * (Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f));
                healthPackAttraction += 2 * (Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f));
                ammoPackAttraction += 2 * (Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f) + Random.Range(-.5f, .5f));
                ...
                APAgentSmith10.MEM.bulletsToDoOneDamageStored = bulletsToDoOneDamage;
                APAgentSmith10.MEM.snagRadiusStored = snagRadius;
                APAgentSmith10.MEM.healthPickupUtilityDangerStored = healthPickupUtilityDanger;
                APAgentSmith10.MEM.healthPickupUtilitySaferStored = healthPickupUtilitySafer;
                APAgentSmith10.MEM.healthPickupUtilitySafeStored = healthPickupUtilitySafe;
                APAgentSmith10.MEM.aggressivenessStored = aggressiveness;
                APAgentSmith10.MEM.healthPackAttractionStored = healthPackAttraction;
                APAgentSmith10.MEM.ammoPackAttractionStored = ammoPackAttraction;

                APAgentSmith10.MEM.alreadyRandomized = true;
            }
            else
            {
                bulletsToDoOneDamage = APAgentSmith10.MEM.bulletsToDoOneDamageStored;
                snagRadius = APAgentSmith10.MEM.snagRadiusStored;
                healthPickupUtilityDanger = APAgentSmith10.MEM.healthPickupUtilityDangerStored;
                healthPickupUtilitySafer = APAgentSmith10.MEM.healthPickupUtilitySaferStored;
                healthPickupUtilitySafe = APAgentSmith10.MEM.healthPickupUtilitySafeStored;
                aggressiveness = APAgentSmith10.MEM.aggressivenessStored;
                healthPackAttraction = APAgentSmith10.MEM.healthPackAttractionStored;
                ammoPackAttraction = APAgentSmith10.MEM.ammoPackAttractionStored;
            }
            ...
            APAgentSmith10.MEM.Stats = Stats;
        }
            ...
            {
                {
                    ...
                    {
                        APAgentSmith10.MEM.spawnPointsVisited = (from point in APAgentSmith10.MEM.spawnPointsVisited
                                                               where point.pos != nearPoints[0].pos
                                                               where point.time > Time.time - 60
                                                               select point).ToList();
                        APAgentSmith10.MEM.spawnPointsVisited.Add(nearPoints[0]);
                    }
                }
                    ...
                    switch (si.sense)
                    {
                        case SensoryInput.eSense.vision:
                            if (si.type == eSensedObjectType.enemy)
                            {
                                ...
                                if (!APAgentSmith10.MEM.observedPositions.Contains(info))
                                    APAgentSmith10.MEM.observedPositions.Add(info);
                            }
                            else if (si.type == eSensedObjectType.item && si.obj is PickUp)
                            {
                                ...
                                if (!APAgentSmith10.MEM.observedSpawnPoints.Contains(info))
                                {
                                    APAgentSmith10.MEM.observedSpawnPoints.Add(info);
                                }
                            }
                            break;
                    }
        ...
        ClosestPosToExploreAndUtility? getExplorationUtility()
        {
            ...
            var pointsToExplore = (from point in iPoints
                                   where !APAgentSmith10.MEM.observedSpawnPoints.Any(info => info.pos == point.pos)
                                   where !APAgentSmith10.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
        ...
        Vector3? Lead(BotPosition bot)
        {
            if (bot.seenLastFrame)
            {
                ...
                var observedData = (from botPosition in APAgentSmith10.MEM.observedPositions
        ...
        PickupToSeekAndUtility? getSeekPickupUtility()
        {
            ...
            var healthPickups = (from point in APAgentSmith10.MEM.observedSpawnPoints
                                 where point.typ == PickUp.eType.health
                                 where !APAgentSmith10.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
                                 select point.pos).ToList();
            var ammoPickups = (from point in APAgentSmith10.MEM.observedSpawnPoints
        ...
        void Hunt(EnemyToHuntAndUtility target)
        {
            ...
            var snaggablesWithinRange = (from point in APAgentSmith10.MEM.observedSpawnPoints
                                         where !APAgentSmith10.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
