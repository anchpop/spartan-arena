...
namespace Andre8
{
    ...
    public class APAgentSmith8 : Agent
    {
        ...
        {
            base.Start();
            APAgentSmith8.MEM.observedPositions = new List<BotPosition>();
            if (APAgentSmith8.MEM.observedSpawnPoints == null)
            {
                APAgentSmith8.MEM.observedSpawnPoints = new List<SpawnPointInfo>();
                APAgentSmith8.MEM.alreadyRandomized = false;
            }
            if (APAgentSmith8.MEM.spawnPointsVisited == null)
            {
                APAgentSmith8.MEM.spawnPointsVisited = new List<SpawnPointVisited>();
                APAgentSmith8.MEM.alreadyRandomized = false;
            }
        ...
        void RandomizeParameters()
        {
            ...
            Stats = "";
            if (!APAgentSmith8.MEM.alreadyRandomized)
            {
                ...
                APAgentSmith8.MEM.bulletsToDoOneDamageStored = bulletsToDoOneDamage;
                APAgentSmith8.MEM.snagRadiusStored = snagRadius;
                APAgentSmith8.MEM.healthPickupUtilityDangerStored = healthPickupUtilityDanger;
                APAgentSmith8.MEM.healthPickupUtilitySaferStored = healthPickupUtilitySafer;
                APAgentSmith8.MEM.healthPickupUtilitySafeStored = healthPickupUtilitySafe;
                APAgentSmith8.MEM.aggressivenessStored = aggressiveness;
                APAgentSmith8.MEM.healthPackAttractionStored = healthPackAttraction;
                APAgentSmith8.MEM.ammoPackAttractionStored = ammoPackAttraction;

                APAgentSmith8.MEM.alreadyRandomized = true;
            }
            else
            {
                bulletsToDoOneDamage = APAgentSmith8.MEM.bulletsToDoOneDamageStored;
                snagRadius = APAgentSmith8.MEM.snagRadiusStored;
                healthPickupUtilityDanger = APAgentSmith8.MEM.healthPickupUtilityDangerStored;
                healthPickupUtilitySafer = APAgentSmith8.MEM.healthPickupUtilitySaferStored;
                healthPickupUtilitySafe = APAgentSmith8.MEM.healthPickupUtilitySafeStored;
                aggressiveness = APAgentSmith8.MEM.aggressivenessStored;
                healthPackAttraction = APAgentSmith8.MEM.healthPackAttractionStored;
                ammoPackAttraction = APAgentSmith8.MEM.ammoPackAttractionStored;
            }
            ...
            {
                {
                    ...
                    {
                        APAgentSmith8.MEM.spawnPointsVisited = (from point in APAgentSmith8.MEM.spawnPointsVisited
                                                               where point.pos != nearPoints[0].pos
                                                               where point.time > Time.time - 60
                                                               select point).ToList();
                        APAgentSmith8.MEM.spawnPointsVisited.Add(nearPoints[0]);
                    }
                }
                    ...
                    switch (si.sense)
                    {
                        case SensoryInput.eSense.vision:
                            if (si.type == eSensedObjectType.enemy)
                            {
                                ...
                                if (!APAgentSmith8.MEM.observedPositions.Contains(info))
                                    APAgentSmith8.MEM.observedPositions.Add(info);
                            }
                            else if (si.type == eSensedObjectType.item && si.obj is PickUp)
                            {
                                ...
                                if (!APAgentSmith8.MEM.observedSpawnPoints.Contains(info))
                                {
                                    APAgentSmith8.MEM.observedSpawnPoints.Add(info);
                                }
                            }
                            break;
                    }
        ...
        ClosestPosToExploreAndUtility? getExplorationUtility()
        {
            ...
            var pointsToExplore = (from point in iPoints
                                   where !APAgentSmith8.MEM.observedSpawnPoints.Any(info => info.pos == point.pos)
                                   where !APAgentSmith8.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
        ...
        Vector3? Lead(BotPosition bot)
        {
            if (bot.seenLastFrame)
            {
                ...
                var observedData = (from botPosition in APAgentSmith8.MEM.observedPositions
        ...
        PickupToSeekAndUtility? getSeekPickupUtility()
        {
            ...
            var healthPickups = (from point in APAgentSmith8.MEM.observedSpawnPoints
                                 where point.typ == PickUp.eType.health
                                 where !APAgentSmith8.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
                                 select point.pos).ToList();
            var ammoPickups = (from point in APAgentSmith8.MEM.observedSpawnPoints
        ...
        void Hunt(EnemyToHuntAndUtility target)
        {
            ...
            var snaggablesWithinRange = (from point in APAgentSmith8.MEM.observedSpawnPoints
                                         where !APAgentSmith8.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
