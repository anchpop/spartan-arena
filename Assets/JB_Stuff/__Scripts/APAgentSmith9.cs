...
namespace Andre9
{
    ...
    public class APAgentSmith9 : Agent
    {
        ...
        {
            base.Start();
            APAgentSmith9.MEM.observedPositions = new List<BotPosition>();
            if (APAgentSmith9.MEM.observedSpawnPoints == null)
            {
                APAgentSmith9.MEM.observedSpawnPoints = new List<SpawnPointInfo>();
                APAgentSmith9.MEM.alreadyRandomized = false;
            }
            if (APAgentSmith9.MEM.spawnPointsVisited == null)
            {
                APAgentSmith9.MEM.spawnPointsVisited = new List<SpawnPointVisited>();
                APAgentSmith9.MEM.alreadyRandomized = false;
            }
        ...
        void RandomizeParameters()
        {
            ...
            Stats = "";
            if (!APAgentSmith9.MEM.alreadyRandomized)
            {
                ...
                APAgentSmith9.MEM.bulletsToDoOneDamageStored = bulletsToDoOneDamage;
                APAgentSmith9.MEM.snagRadiusStored = snagRadius;
                APAgentSmith9.MEM.healthPickupUtilityDangerStored = healthPickupUtilityDanger;
                APAgentSmith9.MEM.healthPickupUtilitySaferStored = healthPickupUtilitySafer;
                APAgentSmith9.MEM.healthPickupUtilitySafeStored = healthPickupUtilitySafe;
                APAgentSmith9.MEM.aggressivenessStored = aggressiveness;
                APAgentSmith9.MEM.healthPackAttractionStored = healthPackAttraction;
                APAgentSmith9.MEM.ammoPackAttractionStored = ammoPackAttraction;

                APAgentSmith9.MEM.alreadyRandomized = true;
            }
            else
            {
                bulletsToDoOneDamage = APAgentSmith9.MEM.bulletsToDoOneDamageStored;
                snagRadius = APAgentSmith9.MEM.snagRadiusStored;
                healthPickupUtilityDanger = APAgentSmith9.MEM.healthPickupUtilityDangerStored;
                healthPickupUtilitySafer = APAgentSmith9.MEM.healthPickupUtilitySaferStored;
                healthPickupUtilitySafe = APAgentSmith9.MEM.healthPickupUtilitySafeStored;
                aggressiveness = APAgentSmith9.MEM.aggressivenessStored;
                healthPackAttraction = APAgentSmith9.MEM.healthPackAttractionStored;
                ammoPackAttraction = APAgentSmith9.MEM.ammoPackAttractionStored;
            }
            ...
            {
                {
                    ...
                    {
                        APAgentSmith9.MEM.spawnPointsVisited = (from point in APAgentSmith9.MEM.spawnPointsVisited
                                                               where point.pos != nearPoints[0].pos
                                                               where point.time > Time.time - 60
                                                               select point).ToList();
                        APAgentSmith9.MEM.spawnPointsVisited.Add(nearPoints[0]);
                    }
                }
                    ...
                    switch (si.sense)
                    {
                        case SensoryInput.eSense.vision:
                            if (si.type == eSensedObjectType.enemy)
                            {
                                ...
                                if (!APAgentSmith9.MEM.observedPositions.Contains(info))
                                    APAgentSmith9.MEM.observedPositions.Add(info);
                            }
                            else if (si.type == eSensedObjectType.item && si.obj is PickUp)
                            {
                                ...
                                if (!APAgentSmith9.MEM.observedSpawnPoints.Contains(info))
                                {
                                    APAgentSmith9.MEM.observedSpawnPoints.Add(info);
                                }
                            }
                            break;
                    }
        ...
        ClosestPosToExploreAndUtility? getExplorationUtility()
        {
            ...
            var pointsToExplore = (from point in iPoints
                                   where !APAgentSmith9.MEM.observedSpawnPoints.Any(info => info.pos == point.pos)
                                   where !APAgentSmith9.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
        ...
        Vector3? Lead(BotPosition bot)
        {
            if (bot.seenLastFrame)
            {
                ...
                var observedData = (from botPosition in APAgentSmith9.MEM.observedPositions
        ...
        PickupToSeekAndUtility? getSeekPickupUtility()
        {
            ...
            var healthPickups = (from point in APAgentSmith9.MEM.observedSpawnPoints
                                 where point.typ == PickUp.eType.health
                                 where !APAgentSmith9.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
                                 select point.pos).ToList();
            var ammoPickups = (from point in APAgentSmith9.MEM.observedSpawnPoints
        ...
        void Hunt(EnemyToHuntAndUtility target)
        {
            ...
            var snaggablesWithinRange = (from point in APAgentSmith9.MEM.observedSpawnPoints
                                         where !APAgentSmith9.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
