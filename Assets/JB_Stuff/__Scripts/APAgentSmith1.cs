...
namespace Andre1
{
    ...
    public class APAgentSmith1 : Agent
    {
        ...
        {
            base.Start();
            APAgentSmith1.MEM.observedPositions = new List<BotPosition>();
            if (APAgentSmith1.MEM.observedSpawnPoints == null)
            {
                APAgentSmith1.MEM.observedSpawnPoints = new List<SpawnPointInfo>();
                APAgentSmith1.MEM.alreadyRandomized = false;
            }
            if (APAgentSmith1.MEM.spawnPointsVisited == null)
            {
                APAgentSmith1.MEM.spawnPointsVisited = new List<SpawnPointVisited>();
                APAgentSmith1.MEM.alreadyRandomized = false;
            }
        ...
        void RandomizeParameters()
        {
            ...
            Stats = "";
            if (!APAgentSmith1.MEM.alreadyRandomized)
            {
                ...
                APAgentSmith1.MEM.bulletsToDoOneDamageStored = bulletsToDoOneDamage;
                APAgentSmith1.MEM.snagRadiusStored = snagRadius;
                APAgentSmith1.MEM.healthPickupUtilityDangerStored = healthPickupUtilityDanger;
                APAgentSmith1.MEM.healthPickupUtilitySaferStored = healthPickupUtilitySafer;
                APAgentSmith1.MEM.healthPickupUtilitySafeStored = healthPickupUtilitySafe;
                APAgentSmith1.MEM.aggressivenessStored = aggressiveness;
                APAgentSmith1.MEM.healthPackAttractionStored = healthPackAttraction;
                APAgentSmith1.MEM.ammoPackAttractionStored = ammoPackAttraction;

                APAgentSmith1.MEM.alreadyRandomized = true;
            }
            else
            {
                bulletsToDoOneDamage = APAgentSmith1.MEM.bulletsToDoOneDamageStored;
                snagRadius = APAgentSmith1.MEM.snagRadiusStored;
                healthPickupUtilityDanger = APAgentSmith1.MEM.healthPickupUtilityDangerStored;
                healthPickupUtilitySafer = APAgentSmith1.MEM.healthPickupUtilitySaferStored;
                healthPickupUtilitySafe = APAgentSmith1.MEM.healthPickupUtilitySafeStored;
                aggressiveness = APAgentSmith1.MEM.aggressivenessStored;
                healthPackAttraction = APAgentSmith1.MEM.healthPackAttractionStored;
                ammoPackAttraction = APAgentSmith1.MEM.ammoPackAttractionStored;
            }
            ...
            {
                {
                    ...
                    {
                        APAgentSmith1.MEM.spawnPointsVisited = (from point in APAgentSmith1.MEM.spawnPointsVisited
                                                               where point.pos != nearPoints[0].pos
                                                               where point.time > Time.time - 60
                                                               select point).ToList();
                        APAgentSmith1.MEM.spawnPointsVisited.Add(nearPoints[0]);
                    }
                }
                    ...
                    switch (si.sense)
                    {
                        case SensoryInput.eSense.vision:
                            if (si.type == eSensedObjectType.enemy)
                            {
                                ...
                                if (!APAgentSmith1.MEM.observedPositions.Contains(info))
                                    APAgentSmith1.MEM.observedPositions.Add(info);
                            }
                            else if (si.type == eSensedObjectType.item && si.obj is PickUp)
                            {
                                ...
                                if (!APAgentSmith1.MEM.observedSpawnPoints.Contains(info))
                                {
                                    APAgentSmith1.MEM.observedSpawnPoints.Add(info);
                                }
                            }
                            break;
                    }
        ...
        ClosestPosToExploreAndUtility? getExplorationUtility()
        {
            ...
            var pointsToExplore = (from point in iPoints
                                   where !APAgentSmith1.MEM.observedSpawnPoints.Any(info => info.pos == point.pos)
                                   where !APAgentSmith1.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
        ...
        Vector3? Lead(BotPosition bot)
        {
            if (bot.seenLastFrame)
            {
                ...
                var observedData = (from botPosition in APAgentSmith1.MEM.observedPositions
        ...
        PickupToSeekAndUtility? getSeekPickupUtility()
        {
            ...
            var healthPickups = (from point in APAgentSmith1.MEM.observedSpawnPoints
                                 where point.typ == PickUp.eType.health
                                 where !APAgentSmith1.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
                                 select point.pos).ToList();
            var ammoPickups = (from point in APAgentSmith1.MEM.observedSpawnPoints
        ...
        void Hunt(EnemyToHuntAndUtility target)
        {
            ...
            var snaggablesWithinRange = (from point in APAgentSmith1.MEM.observedSpawnPoints
                                         where !APAgentSmith1.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
