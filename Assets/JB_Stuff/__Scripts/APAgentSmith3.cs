...
namespace Andre3
{
    ...
    public class APAgentSmith3 : Agent
    {
        ...
        float bulletsToDoOneDamage = 3.4f;
        ...
        {
            base.Start();
            APAgentSmith3.MEM.observedPositions = new List<BotPosition>();
            if (APAgentSmith3.MEM.observedSpawnPoints == null)
            {
                APAgentSmith3.MEM.observedSpawnPoints = new List<SpawnPointInfo>();
                APAgentSmith3.MEM.alreadyRandomized = false;
            }
            if (APAgentSmith3.MEM.spawnPointsVisited == null)
            {
                APAgentSmith3.MEM.spawnPointsVisited = new List<SpawnPointVisited>();
                APAgentSmith3.MEM.alreadyRandomized = false;
            }
        ...
        void RandomizeParameters()
        {
            ...
            Stats = "";
            if (!APAgentSmith3.MEM.alreadyRandomized)
            {
                ...
                APAgentSmith3.MEM.bulletsToDoOneDamageStored = bulletsToDoOneDamage;
                APAgentSmith3.MEM.snagRadiusStored = snagRadius;
                APAgentSmith3.MEM.healthPickupUtilityDangerStored = healthPickupUtilityDanger;
                APAgentSmith3.MEM.healthPickupUtilitySaferStored = healthPickupUtilitySafer;
                APAgentSmith3.MEM.healthPickupUtilitySafeStored = healthPickupUtilitySafe;
                APAgentSmith3.MEM.aggressivenessStored = aggressiveness;
                APAgentSmith3.MEM.healthPackAttractionStored = healthPackAttraction;
                APAgentSmith3.MEM.ammoPackAttractionStored = ammoPackAttraction;

                APAgentSmith3.MEM.alreadyRandomized = true;
            }
            else
            {
                bulletsToDoOneDamage = APAgentSmith3.MEM.bulletsToDoOneDamageStored;
                snagRadius = APAgentSmith3.MEM.snagRadiusStored;
                healthPickupUtilityDanger = APAgentSmith3.MEM.healthPickupUtilityDangerStored;
                healthPickupUtilitySafer = APAgentSmith3.MEM.healthPickupUtilitySaferStored;
                healthPickupUtilitySafe = APAgentSmith3.MEM.healthPickupUtilitySafeStored;
                aggressiveness = APAgentSmith3.MEM.aggressivenessStored;
                healthPackAttraction = APAgentSmith3.MEM.healthPackAttractionStored;
                ammoPackAttraction = APAgentSmith3.MEM.ammoPackAttractionStored;
            }
            ...
            {
                {
                    ...
                    {
                        APAgentSmith3.MEM.spawnPointsVisited = (from point in APAgentSmith3.MEM.spawnPointsVisited
                                                               where point.pos != nearPoints[0].pos
                                                               where point.time > Time.time - 60
                                                               select point).ToList();
                        APAgentSmith3.MEM.spawnPointsVisited.Add(nearPoints[0]);
                    }
                }
                    ...
                    switch (si.sense)
                    {
                        case SensoryInput.eSense.vision:
                            if (si.type == eSensedObjectType.enemy)
                            {
                                ...
                                if (!APAgentSmith3.MEM.observedPositions.Contains(info))
                                    APAgentSmith3.MEM.observedPositions.Add(info);
                            }
                            else if (si.type == eSensedObjectType.item && si.obj is PickUp)
                            {
                                ...
                                if (!APAgentSmith3.MEM.observedSpawnPoints.Contains(info))
                                {
                                    APAgentSmith3.MEM.observedSpawnPoints.Add(info);
                                }
                            }
                            break;
                    }
        ...
        ClosestPosToExploreAndUtility? getExplorationUtility()
        {
            ...
            var pointsToExplore = (from point in iPoints
                                   where !APAgentSmith3.MEM.observedSpawnPoints.Any(info => info.pos == point.pos)
                                   where !APAgentSmith3.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
        ...
        Vector3? Lead(BotPosition bot)
        {
            if (bot.seenLastFrame)
            {
                ...
                var observedData = (from botPosition in APAgentSmith3.MEM.observedPositions
        ...
        PickupToSeekAndUtility? getSeekPickupUtility()
        {
            ...
            var healthPickups = (from point in APAgentSmith3.MEM.observedSpawnPoints
                                 where point.typ == PickUp.eType.health
                                 where !APAgentSmith3.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
                                 select point.pos).ToList();
            var ammoPickups = (from point in APAgentSmith3.MEM.observedSpawnPoints
        ...
        void Hunt(EnemyToHuntAndUtility target)
        {
            ...
            var snaggablesWithinRange = (from point in APAgentSmith3.MEM.observedSpawnPoints
                                         where !APAgentSmith3.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
