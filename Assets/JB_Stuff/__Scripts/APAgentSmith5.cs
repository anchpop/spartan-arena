...
namespace Andre5
{
    ...
    public class APAgentSmith5 : Agent
    {
        ...
        {
            base.Start();
            APAgentSmith5.MEM.observedPositions = new List<BotPosition>();
            if (APAgentSmith5.MEM.observedSpawnPoints == null)
            {
                APAgentSmith5.MEM.observedSpawnPoints = new List<SpawnPointInfo>();
                APAgentSmith5.MEM.alreadyRandomized = false;
            }
            if (APAgentSmith5.MEM.spawnPointsVisited == null)
            {
                APAgentSmith5.MEM.spawnPointsVisited = new List<SpawnPointVisited>();
                APAgentSmith5.MEM.alreadyRandomized = false;
            }
        ...
        void RandomizeParameters()
        {
            ...
            Stats = "";
            if (!APAgentSmith5.MEM.alreadyRandomized)
            {
                ...
                APAgentSmith5.MEM.bulletsToDoOneDamageStored = bulletsToDoOneDamage;
                APAgentSmith5.MEM.snagRadiusStored = snagRadius;
                APAgentSmith5.MEM.healthPickupUtilityDangerStored = healthPickupUtilityDanger;
                APAgentSmith5.MEM.healthPickupUtilitySaferStored = healthPickupUtilitySafer;
                APAgentSmith5.MEM.healthPickupUtilitySafeStored = healthPickupUtilitySafe;
                APAgentSmith5.MEM.aggressivenessStored = aggressiveness;
                APAgentSmith5.MEM.healthPackAttractionStored = healthPackAttraction;
                APAgentSmith5.MEM.ammoPackAttractionStored = ammoPackAttraction;

                APAgentSmith5.MEM.alreadyRandomized = true;
            }
            else
            {
                bulletsToDoOneDamage = APAgentSmith5.MEM.bulletsToDoOneDamageStored;
                snagRadius = APAgentSmith5.MEM.snagRadiusStored;
                healthPickupUtilityDanger = APAgentSmith5.MEM.healthPickupUtilityDangerStored;
                healthPickupUtilitySafer = APAgentSmith5.MEM.healthPickupUtilitySaferStored;
                healthPickupUtilitySafe = APAgentSmith5.MEM.healthPickupUtilitySafeStored;
                aggressiveness = APAgentSmith5.MEM.aggressivenessStored;
                healthPackAttraction = APAgentSmith5.MEM.healthPackAttractionStored;
                ammoPackAttraction = APAgentSmith5.MEM.ammoPackAttractionStored;
            }
            ...
            {
                {
                    ...
                    {
                        APAgentSmith5.MEM.spawnPointsVisited = (from point in APAgentSmith5.MEM.spawnPointsVisited
                                                               where point.pos != nearPoints[0].pos
                                                               where point.time > Time.time - 60
                                                               select point).ToList();
                        APAgentSmith5.MEM.spawnPointsVisited.Add(nearPoints[0]);
                    }
                }
                    ...
                    switch (si.sense)
                    {
                        case SensoryInput.eSense.vision:
                            if (si.type == eSensedObjectType.enemy)
                            {
                                ...
                                if (!APAgentSmith5.MEM.observedPositions.Contains(info))
                                    APAgentSmith5.MEM.observedPositions.Add(info);
                            }
                            else if (si.type == eSensedObjectType.item && si.obj is PickUp)
                            {
                                ...
                                if (!APAgentSmith5.MEM.observedSpawnPoints.Contains(info))
                                {
                                    APAgentSmith5.MEM.observedSpawnPoints.Add(info);
                                }
                            }
                            break;
                    }
        ...
        ClosestPosToExploreAndUtility? getExplorationUtility()
        {
            ...
            var pointsToExplore = (from point in iPoints
                                   where !APAgentSmith5.MEM.observedSpawnPoints.Any(info => info.pos == point.pos)
                                   where !APAgentSmith5.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
        ...
        Vector3? Lead(BotPosition bot)
        {
            if (bot.seenLastFrame)
            {
                ...
                var observedData = (from botPosition in APAgentSmith5.MEM.observedPositions
        ...
        PickupToSeekAndUtility? getSeekPickupUtility()
        {
            ...
            var healthPickups = (from point in APAgentSmith5.MEM.observedSpawnPoints
                                 where point.typ == PickUp.eType.health
                                 where !APAgentSmith5.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
                                 select point.pos).ToList();
            var ammoPickups = (from point in APAgentSmith5.MEM.observedSpawnPoints
        ...
        void Hunt(EnemyToHuntAndUtility target)
        {
            ...
            var snaggablesWithinRange = (from point in APAgentSmith5.MEM.observedSpawnPoints
                                         where !APAgentSmith5.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
