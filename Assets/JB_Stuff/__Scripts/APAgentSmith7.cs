...
namespace Andre7
{
    ...
    public class APAgentSmith7 : Agent
    {
        ...
        {
            base.Start();
            APAgentSmith7.MEM.observedPositions = new List<BotPosition>();
            if (APAgentSmith7.MEM.observedSpawnPoints == null)
            {
                APAgentSmith7.MEM.observedSpawnPoints = new List<SpawnPointInfo>();
                APAgentSmith7.MEM.alreadyRandomized = false;
            }
            if (APAgentSmith7.MEM.spawnPointsVisited == null)
            {
                APAgentSmith7.MEM.spawnPointsVisited = new List<SpawnPointVisited>();
                APAgentSmith7.MEM.alreadyRandomized = false;
            }
        ...
        void RandomizeParameters()
        {
            ...
            Stats = "";
            if (!APAgentSmith7.MEM.alreadyRandomized)
            {
                ...
                APAgentSmith7.MEM.bulletsToDoOneDamageStored = bulletsToDoOneDamage;
                APAgentSmith7.MEM.snagRadiusStored = snagRadius;
                APAgentSmith7.MEM.healthPickupUtilityDangerStored = healthPickupUtilityDanger;
                APAgentSmith7.MEM.healthPickupUtilitySaferStored = healthPickupUtilitySafer;
                APAgentSmith7.MEM.healthPickupUtilitySafeStored = healthPickupUtilitySafe;
                APAgentSmith7.MEM.aggressivenessStored = aggressiveness;
                APAgentSmith7.MEM.healthPackAttractionStored = healthPackAttraction;
                APAgentSmith7.MEM.ammoPackAttractionStored = ammoPackAttraction;

                APAgentSmith7.MEM.alreadyRandomized = true;
            }
            else
            {
                bulletsToDoOneDamage = APAgentSmith7.MEM.bulletsToDoOneDamageStored;
                snagRadius = APAgentSmith7.MEM.snagRadiusStored;
                healthPickupUtilityDanger = APAgentSmith7.MEM.healthPickupUtilityDangerStored;
                healthPickupUtilitySafer = APAgentSmith7.MEM.healthPickupUtilitySaferStored;
                healthPickupUtilitySafe = APAgentSmith7.MEM.healthPickupUtilitySafeStored;
                aggressiveness = APAgentSmith7.MEM.aggressivenessStored;
                healthPackAttraction = APAgentSmith7.MEM.healthPackAttractionStored;
                ammoPackAttraction = APAgentSmith7.MEM.ammoPackAttractionStored;
            }
            ...
            {
                {
                    ...
                    {
                        APAgentSmith7.MEM.spawnPointsVisited = (from point in APAgentSmith7.MEM.spawnPointsVisited
                                                               where point.pos != nearPoints[0].pos
                                                               where point.time > Time.time - 60
                                                               select point).ToList();
                        APAgentSmith7.MEM.spawnPointsVisited.Add(nearPoints[0]);
                    }
                }
                    ...
                    switch (si.sense)
                    {
                        case SensoryInput.eSense.vision:
                            if (si.type == eSensedObjectType.enemy)
                            {
                                ...
                                if (!APAgentSmith7.MEM.observedPositions.Contains(info))
                                    APAgentSmith7.MEM.observedPositions.Add(info);
                            }
                            else if (si.type == eSensedObjectType.item && si.obj is PickUp)
                            {
                                ...
                                if (!APAgentSmith7.MEM.observedSpawnPoints.Contains(info))
                                {
                                    APAgentSmith7.MEM.observedSpawnPoints.Add(info);
                                }
                            }
                            break;
                    }
        ...
        ClosestPosToExploreAndUtility? getExplorationUtility()
        {
            ...
            var pointsToExplore = (from point in iPoints
                                   where !APAgentSmith7.MEM.observedSpawnPoints.Any(info => info.pos == point.pos)
                                   where !APAgentSmith7.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
        ...
        Vector3? Lead(BotPosition bot)
        {
            if (bot.seenLastFrame)
            {
                ...
                var observedData = (from botPosition in APAgentSmith7.MEM.observedPositions
        ...
        PickupToSeekAndUtility? getSeekPickupUtility()
        {
            ...
            var healthPickups = (from point in APAgentSmith7.MEM.observedSpawnPoints
                                 where point.typ == PickUp.eType.health
                                 where !APAgentSmith7.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
                                 select point.pos).ToList();
            var ammoPickups = (from point in APAgentSmith7.MEM.observedSpawnPoints
        ...
        void Hunt(EnemyToHuntAndUtility target)
        {
            ...
            var snaggablesWithinRange = (from point in APAgentSmith7.MEM.observedSpawnPoints
                                         where !APAgentSmith7.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
