...
namespace Andre4
{
    ...
    public class APAgentSmith4 : Agent
    {
        ...
        float bulletsToDoOneDamage = 3.5f;
        float snagRadius = 5.1f;
        float healthPickupUtilityDanger = 15;
        float healthPickupUtilitySafer = 7.13f;
        float healthPickupUtilitySafe = 1.28f;
        float aggressiveness = 1.3f;
        float healthPackAttraction = 3.5f;
        float ammoPackAttraction = 1.5f;

        private void Start()
        {
            base.Start();
            APAgentSmith4.MEM.observedPositions = new List<BotPosition>();
            if (APAgentSmith4.MEM.observedSpawnPoints == null)
            {
                APAgentSmith4.MEM.observedSpawnPoints = new List<SpawnPointInfo>();
                APAgentSmith4.MEM.alreadyRandomized = false;
            }
            if (APAgentSmith4.MEM.spawnPointsVisited == null)
            {
                APAgentSmith4.MEM.spawnPointsVisited = new List<SpawnPointVisited>();
                APAgentSmith4.MEM.alreadyRandomized = false;
            }
        ...
        void RandomizeParameters()
        {
            ...
            Stats = "";
            if (!APAgentSmith4.MEM.alreadyRandomized)
            {
                ...
                APAgentSmith4.MEM.bulletsToDoOneDamageStored = bulletsToDoOneDamage;
                APAgentSmith4.MEM.snagRadiusStored = snagRadius;
                APAgentSmith4.MEM.healthPickupUtilityDangerStored = healthPickupUtilityDanger;
                APAgentSmith4.MEM.healthPickupUtilitySaferStored = healthPickupUtilitySafer;
                APAgentSmith4.MEM.healthPickupUtilitySafeStored = healthPickupUtilitySafe;
                APAgentSmith4.MEM.aggressivenessStored = aggressiveness;
                APAgentSmith4.MEM.healthPackAttractionStored = healthPackAttraction;
                APAgentSmith4.MEM.ammoPackAttractionStored = ammoPackAttraction;

                APAgentSmith4.MEM.alreadyRandomized = true;
            }
            else
            {
                bulletsToDoOneDamage = APAgentSmith4.MEM.bulletsToDoOneDamageStored;
                snagRadius = APAgentSmith4.MEM.snagRadiusStored;
                healthPickupUtilityDanger = APAgentSmith4.MEM.healthPickupUtilityDangerStored;
                healthPickupUtilitySafer = APAgentSmith4.MEM.healthPickupUtilitySaferStored;
                healthPickupUtilitySafe = APAgentSmith4.MEM.healthPickupUtilitySafeStored;
                aggressiveness = APAgentSmith4.MEM.aggressivenessStored;
                healthPackAttraction = APAgentSmith4.MEM.healthPackAttractionStored;
                ammoPackAttraction = APAgentSmith4.MEM.ammoPackAttractionStored;
            }
            ...
            {
                {
                    ...
                    {
                        APAgentSmith4.MEM.spawnPointsVisited = (from point in APAgentSmith4.MEM.spawnPointsVisited
                                                               where point.pos != nearPoints[0].pos
                                                               where point.time > Time.time - 60
                                                               select point).ToList();
                        APAgentSmith4.MEM.spawnPointsVisited.Add(nearPoints[0]);
                    }
                }
                    ...
                    switch (si.sense)
                    {
                        case SensoryInput.eSense.vision:
                            if (si.type == eSensedObjectType.enemy)
                            {
                                ...
                                if (!APAgentSmith4.MEM.observedPositions.Contains(info))
                                    APAgentSmith4.MEM.observedPositions.Add(info);
                            }
                            else if (si.type == eSensedObjectType.item && si.obj is PickUp)
                            {
                                ...
                                if (!APAgentSmith4.MEM.observedSpawnPoints.Contains(info))
                                {
                                    APAgentSmith4.MEM.observedSpawnPoints.Add(info);
                                }
                            }
                            break;
                    }
        ...
        ClosestPosToExploreAndUtility? getExplorationUtility()
        {
            ...
            var pointsToExplore = (from point in iPoints
                                   where !APAgentSmith4.MEM.observedSpawnPoints.Any(info => info.pos == point.pos)
                                   where !APAgentSmith4.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
        ...
        Vector3? Lead(BotPosition bot)
        {
            if (bot.seenLastFrame)
            {
                ...
                var observedData = (from botPosition in APAgentSmith4.MEM.observedPositions
        ...
        PickupToSeekAndUtility? getSeekPickupUtility()
        {
            ...
            var healthPickups = (from point in APAgentSmith4.MEM.observedSpawnPoints
                                 where point.typ == PickUp.eType.health
                                 where !APAgentSmith4.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
                                 select point.pos).ToList();
            var ammoPickups = (from point in APAgentSmith4.MEM.observedSpawnPoints
        ...
        void Hunt(EnemyToHuntAndUtility target)
        {
            ...
            var snaggablesWithinRange = (from point in APAgentSmith4.MEM.observedSpawnPoints
                                         where !APAgentSmith4.MEM.spawnPointsVisited.Any(info => info.pos == point.pos)
